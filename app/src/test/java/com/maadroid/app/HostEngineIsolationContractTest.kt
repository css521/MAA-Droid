...
class HostEngineIsolationContractTest {
    ...
    private val arknightsPackages = listOf(
        "com.maadroid.app.maa.",
        "com.maadroid.app.data.resource.",
        "com.maadroid.app.data.achievement.",
        "com.maadroid.app.data.repository.",
        "com.maadroid.app.presentation.view.panel.",
        // 已抽成 engine:arknights 模块的部分：宿主引用它们同样算耦合，
        // 只是这些引用现在合法（app → engine:arknights），计数用于追踪剩余搬迁量
        "com.maadroid.app.engine.arknights.",
    )

    /** 混杂包里按类名精确点出的方舟类型（尚未抽出的） */
    private val arknightsClasses = listOf(
        "com.maadroid.app.data.config.MaaPathConfig",
    )
    ...
    private val runtimeAssemblyImports = setOf(
        "com.maadroid.app.engine.arknights.ArknightsEngine",
        "com.maadroid.app.engine.arknights.ArknightsDeviceAdapter",
        "com.maadroid.app.engine.arknights.MaaResourcePreparation",
        "com.maadroid.app.engine.arknights.MaaRunOptions",
    )
    ...
    private val arknightsToHost = mapOf(
        ...
        "com.maadroid.app.presentation.components." to 1,
        "com.maadroid.app.theme." to 2,
        ...
        "com.maadroid.app.utils.i18n." to 4,
        // → 方舟侧改构造函数注入即可消除，非前置
        "com.maadroid.app.presentation.viewmodel." to 15,
        // → 随 TaskChainState 一起迁入方舟（P2）。MaaPathConfig 已解耦（10→9），
        // 剩下的主要是 ActivityManager 对 TaskChainState 的依赖
        "com.maadroid.app.data.preferences." to 9, // 含方舟 provider 的 prepare 选项快照
        "com.maadroid.app.presentation.state." to 1,
        // 已为 0：一旦出现即是新增的反向依赖
        "com.maadroid.app.koin." to 0,
    )
    ...
    private companion object {
        const val APP_SOURCE_ROOT = "src/main/java/com/maadroid/app"
