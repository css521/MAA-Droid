...
class TaskConfigWireFormatTest {
    ...
    fun configDiscriminatorIsFullyQualifiedClassName() {
        ...
        assertTrue(
            "判别符应含全限定类名，实际: $text",
            text.contains("com.maadroid.app.data.model.FightConfig"),
        )
    }
    ...
    fun everyTaskConfigRoundTripsThroughItsCurrentDiscriminator() {
        ...
        for (cfg in configs) {
            ...
            val text = json.encodeToString(node)
            val expected = "com.maadroid.app.data.model.${cfg::class.simpleName}"
        ...
        val stored = """
            {
              ...
              "name": "刷理智",
              "config": {
                "type": "com.maadroid.app.data.model.FightConfig"
              }
            }
        """.trimIndent()
    ...
    fun discriminatorComesFromSerialNameNotFromPackage() {
        ...
        for (cfg in configs) {
            ...
            assertEquals(
                ...
                "com.maadroid.app.data.model.${cls.simpleName}",
                pinned!!.value,
            )
            // 且实际写出来的就是它
