...
This document describes how to trigger MAA Droid to run a specific task profile from external apps (such as MacroDroid or Tasker) via an Intent / `am` command.
...
- Time-share a single device between MAA Droid and other automation tasks (e.g. OA check-in)
- Control the exact task start time through a third-party scheduler

Typical flow:

```
Timer fires -> Unlock screen -> am start launches the target profile -> MAA Droid runs the task -> Webhook callback -> Lock screen
```
...
1. Open MAA Droid and go to the **Background Task** page
...
3. Enable **Use Root**
4. To auto-lock the screen after the task finishes, combine it with MAA Droid's **Webhook**: set the Webhook URL in MAA Droid settings, and have MacroDroid listen for that callback to perform the lock action
