...
- **OCR**（上游 22 处）：饰品名、主题卡包、队伍人数、现金。缺了会让镜牢商店与跨层选饰品退化到只走模板兜底。
  - 模型已随资源包下发（PP-OCRv5 det 4.6 MB + rec 15.9 MB）。**字符表嵌在 rec 模型的 `metadata_props["character"]` 里**（18383 个字符），不必另外向 PaddleOCR 取字典，也就不存在「字典与模型版本对不上」的风险。
  - CTC 解码与检测框合并已实现并测试（`ocr/CtcDecoder`、`ocr/TextMerge`）：字符表按 blank 在前、空格在后组装成 18385 类，与该模型输出层维度逐一核对过。
  - **尚缺 DB 检测后处理与模型接线**，故 `detectText` / `findText` 目前仍返回空表。
