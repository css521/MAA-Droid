...
本轮执行分支回归：App 715、Limbus 234、core/remote 66，合计 1015 项 JVM 测试通过，无失败或跳过。另在 macOS OpenCV 4.9 上运行 8 项算法 native 测试和 5 项取帧/缓存/裁剪集成测试；Android 编译使用 OpenCV 4.11。三个原始分类模型也通过主机 OpenCV DNN 的输入/输出维度及有限数值检查。主机验证不代表 Android ONNX Runtime 或游戏实机流程已验收。
