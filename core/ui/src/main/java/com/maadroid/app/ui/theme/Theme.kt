package com.maadroid.app.ui.theme
 ...
 * 由 [MaaDroidTheme] 统一下发；未进入 App 主题时为 null。
 */
 ...
 *
 * 用于弹窗（[com.maadroid.app.presentation.components.AdaptiveTaskPromptDialog] 等）——
...
fun MaaDroidTheme(
    ...
    val colorScheme: ColorScheme = remember(themeMode, useSystemMonetColor, isDarkTheme, context) {
        when {
            ...
            // Otherwise fall back to the built-in blue palette
                ...
                ThemeMode.SYSTEM -> if (systemDarkTheme) BlueDark else BlueLight
                ThemeMode.WHITE -> BlueLight
                ThemeMode.DARK -> BlueDark
                ThemeMode.PURE_DARK -> BluePureDark
            }
        }
    }
    ...
    CompositionLocalProvider(
        ...
        // 供宿主在主题之上叠加自己的色板（如日志级别配色）。
        // core:ui 不认识宿主的日志模型，所以只把「当前是否暗色」告诉外面，
        // 由宿主自己决定拿它做什么
 ...
 * 当前主题是否为暗色。
 *
 * 存在的理由：宿主要在主题之上叠加自己的色板（日志级别配色依赖 isDark），而
 * `core:ui` 不认识宿主的 `LogColorRole` 等模型 —— 于是只暴露这一个布尔量，
 * 由宿主自行组合。反过来把日志色板塞进 core:ui 会让它反向依赖宿主的数据模型。
 */
